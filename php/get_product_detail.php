...
$statement->bindParam(":id", $_SESSION["product_id"], PDO::PARAM_STR);
$statement->execute();
...
if ($statement->rowCount() > 0) {
    ...
    foreach ($result as $row) {
        ...
        $json .= '{"id":"' . $row->id . '","title":"' . $row->title . '","price":"' . $row->price . '","image":"' . $row->image . '","description":"' . $row->description . '","category":"' . $row->category . '","currency":"' . $_SESSION["symbol"] . '"}';
